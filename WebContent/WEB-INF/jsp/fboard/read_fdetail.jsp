<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page import="model.freply.FreplyDTO"%>
<%@ page import="model.freply.FreplyDAO"%>
...
<link rel="stylesheet" type="text/css"
	href="${pageContext.request.contextPath }/css/board/read_board.css">
<link rel="stylesheet"
...
<body>
	<input type="number" name="f_no"
		value="<%=request.getAttribute("fboardNo")%>" style="display: none;">
	<div id="root">
		<main id="contents">
			...
			<section class="fixed-area">
				<header class="header-wrap">
					...
					<div class="left-wrap">
						<a href="http://localhost:8081/MovieMate/freeCommunityController"><button class="back-btn">
							<i class="fa-solid fa-angle-left fa-xl" style="color: #efefef"
								id="icon"></i>
						</button></a>
					</div>
        <div class="header-button">
          <a href="http://localhost:8081/MovieMate/mainController"><button><i class="fa-solid fa-house-chimney fa-xl" style="color:#efefef"></i></button></a>
          <button id="moreBtn"
        		<%String userNick = (String) session.getAttribute("userNick"); %>
        		<%System.out.println(userNick + " | " + request.getAttribute("m_nick")); %>
        		<%if(userNick != null) {%>
		<%if(!userNick.equals(request.getAttribute("m_nick")) || !userNick.equals("관리자")) { %>
          style="display: none;"
        <%}} %>
          ><i  class="fa-solid fa-ellipsis-vertical fa-xl" style="color: #ffffff;"></i></button>
			...
			<section class="content-area">
					...
					<div class="section-divider">
						<div class="post-info-user">
							<div class="post-author-wrap">
								<input type="text" name="nick" class="user-nickname"
									style="display: none;"> <span class="user-nickname"><%=request.getAttribute("userId")%></span>
							</div>
							<div class="post-info-date">
								<span class="post-date" data-fboard-regdate="${fboardRegDate}"></span>
							</div>
						</div>
						<div class="post-body">
							<h1 class="post-title"><%=request.getAttribute("fboardTitle")%></h1>
							<div class="post-content">
								<div class="post-editor post-editor--readonly">
									<div class="post-editor-main">
										<div class="main-writer">
											<span id="content1"><%=request.getAttribute("fboardContent")%></span>
											<br>
										</div>
									</div>
								</div>
						...
						<div class="post-footer">
							<div class="badge">
								<i class="fa-solid fa-comment" style="color: #98a4b7;"></i> <span>${result.size()}</span>
							</div>
						</div>
					</div>
					<div class="comments-area">
						<h2 class="subtitle">
							댓글<span class="commentCount">${result.size()}</span>
						</h2>
						<ul class="comment-list-wrap" id="commentList">
							<%
								ArrayList<FreplyDTO> result = (ArrayList<FreplyDTO>) request.getAttribute("result");
							%>
							<%
								for (FreplyDTO reply : result) {
							%>
								...
								<div class="comment">
									<div class="comment-header">
										...
										<div class="right-area">
											<span class="date"
												data-comment-regdate="<%=reply.getFreply_regdate()%>"></span>
											<button id="moreBtnComment" class="moreBtnComment">
												<i class="fa-solid fa-ellipsis-vertical fa-l"
													style="color: #586a85; margin-top: 2px;"
													onclick="test('<%=reply.getFreply_content()%>','<%=reply.getFreply_no()%>')"></i>
											</button>
										</div>
									</div>
									<p class="contents-area" id="content"><%=reply.getFreply_content()%></p>
								</div> <input id="test" style="display: none" value="">
								<hr class="divider">
									...
									<div class="outerModal">
										<div class="innerModal" style="position: relative">
											...
											<ul class="contents-wrap">
												<li><button class="modifyComment"
														onclick="modifyComments()">수정하기</button></li>
				...
				<div class="comment-input-area" id="commentInputArea">
					<form method="post"
						action="/ReadFreeDetailController?no=<%=request.getAttribute("fboardNo")%>"
						id="insertComment">
						<input type="hidden" name="f_no"
							value="<%=request.getAttribute("fboardNo")%>">
						<div class="input-wrap">
							<div class="textarea-wrap">
								<textarea id="commentTextarea" class="commentTextarea"
									name="comments" placeholder="댓글을 작성해 주세요" rows="1"
									style="-limit: 5; height: auto;" class="origin"></textarea>
								<div id="character-count"></div>
							</div>

							<button type="submit" class="addCommentBtn" id="addCommentBtn"
								name="submit">등록</button>
							<button type="button" class="cancel" style="display: none;">취소</button>
							<button type="submit" class="submit active" id="submit"
		...
		<div id="postDeleteModal" class="confirm-modal-container">
			...
			<div class="confirm-modal-footer">
				<div class="footer">
					...
					<button id="postConfirmButton" class="blue-btn"
		...
		<div id="commentDeleteModal" class="confirm-modal-container">
			...
			<div class="confirm-modal-footer">
				<div class="footer">
					...
					<button id="commentConfirmButton" class="blue-btn"
...
$(document).ready(function() {
 ...
 document.addEventListener("DOMContentLoaded", function () {
  ...
  function modifyContents() {
 	  // 게시물 수정 페이지로 이동
 	  var fboardNo = <%=request.getAttribute("fboardNo")%>;
 	  var contextPath = "${pageContext.request.contextPath}";
 	  var url = contextPath + "/UpdateFreeController?no=" + fboardNo;
 	  window.location.href = url;
 	}
  //게시물 삭제
  function deleteContents(){
	  var f_no= $("input[name='f_no']").val();
	  $.ajax({
		    type: "POST",
		    url: "${pageContext.request.contextPath}/DeleteFreeController",
		    data: {f_no: f_no, fb_isshow: 'N'},
		    success: function(response) {
		      console.log("게시글이 삭제되었습니다.");
		      window.location.href = "${pageContext.request.contextPath}/InsertFreeController";
		    },
  ...
  $('.addCommentBtn').click(function (event) {
      ...
      var fboardNo = $('input[name="f_no"]').val();
      ...
      $.ajax({
    	    type: "POST",
    	    url: "${pageContext.request.contextPath}/InsertFreeReplyController", 
    	    data: formData, 
    	    dataType: 'json',
...
function test(content,rno) {
 ...
 function modifyComments() {
   ...
   $('.submit').click(function(event) {
       ...
       $.ajax({
         type: "POST",
         url: "${pageContext.request.contextPath}/UpdateFreeReplyController",
         data: {
           rno: num,
 ...
 function deleteComments(){
	  var rno = num;
	  var no = $('input[name="f_no"]').val();
	  
   $.ajax({
		    type: "POST",
		    url: "${pageContext.request.contextPath}/DeleteFreeReplyController",
		    data: { no: no, rno: rno, fr_isshow: 'N' },
		    success: function(response) {
		     	window.location.reload();
		    },
		    error: function() {
  ...
  var postDate = new Date(postDateElement.getAttribute("data-fboard-regdate"));
