...
<body>
<form id="insertForm" action="http://localhost:8081/MovieMate/ReadMateDetailController" method="post">
<input type="number" name="no" value="<%=request.getAttribute("boardNo") %>" style="display:none;">
  <input type="text" name="nick" class="user-nickname" value="<%= request.getAttribute("m_nick") %>" style="display:none;">
  <div id="root">
    <main id="contents">
      ...
      <div class="content-area">
        <div class="post-editor">
          <div class="editor-header">
            <div class="textarea1">
              <textarea name="title" maxlength="100" placeholder="제목을 입력해주세요." autofocus="autofocus" row="1" style="height: 21px;"><%= request.getAttribute("boardTitle") %></textarea>
            </div>
          </div>
  ...
  $(document).ready(function () {
      ...
      $('.btn').click(function (event) {
          ...
          var no = $('input[name="no"]').val(); // 게시글 번호
          ...
          $.ajax({
            type: "POST",
            url: "${pageContext.request.contextPath}/UpdateMateController", // 수정된 URL로 변경
            data: formData,
            dataType: 'json', // 응답 데이터 형식을 JSON으로 지정
            success: function (result) {
              alert("수정되었습니다.");
              // 수정 후 처리할 작업 추가 (예: 페이지 리로드 등)
              window.location.href = "${pageContext.request.contextPath}/ReadMateDetailController?no=" + no;
            },
